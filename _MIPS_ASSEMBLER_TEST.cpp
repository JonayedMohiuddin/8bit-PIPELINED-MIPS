...
vector<string> updateLabelMaps(vector<string> lines)
{
    ...
    for (string line : lines)
    {
        ...
        {
            // replace jal with setting ra reg and j to label
            // jal format : jal string_label
            if(tokens[0] == "jal")
            {
                if(tokens.size() < 2)
                {
                    cerr << "!! Invalid number of arguments for jal instruction." << endl;
                    exit(1);
                }
                ...
                updatedLines.push_back("addi $ra, $zero, " + to_string(lineNumber + 1));
                updatedLines.push_back("j " + label);
            }
            else
            {
